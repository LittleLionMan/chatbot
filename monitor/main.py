...
async def _fetch_article_text(client: httpx.AsyncClient, url: str) -> str:
    ...
    try:
        resp = await client.get(
            url, timeout=ARTICLE_FETCH_TIMEOUT, follow_redirects=True
        )
...
async def _get_active_configs(pool: asyncpg.Pool) -> list[dict]:
    rows = await pool.fetch(
        "SELECT * FROM monitor_configs WHERE is_active = TRUE ORDER BY id"
    )
    return [dict(r) for r in rows]


async def _resolve_watchlist(
    pool: asyncpg.Pool, config: dict
        ...
        try:
            ...
            if isinstance(entries, list):
                for entry in entries:
                    ...
                    if ticker:
                        result[ticker] = (name, date)
        except Exception as e:
            logger.warning("json_array_overview parse failed: %s", e)
...
async def _get_seen(pool: asyncpg.Pool, config_id: int) -> set[str]:
    rows = await pool.fetch(
        "SELECT fingerprint FROM monitor_seen WHERE config_id = $1", config_id
    )
...
async def _enqueue_trigger(
    pool: asyncpg.Pool, target_agent: str, payload: dict
) -> None:
    await pool.execute(
        """
...
async def _fetch_feed(client: httpx.AsyncClient, url: str) -> list[dict]:
    try:
        ...
        for item in root.findall(".//item"):
            ...
            if title and link:
                items.append(
                    {"title": title, "url": link, "published": pub, "source": source}
                )
        return items
    except Exception as e:
...
async def _process_feed_items(
    ...
    for article in articles:
        ...
        if article_text and not _matches_keywords(
            article["title"], article_text, keywords
        ):
...
async def _run_rss_config(pool: asyncpg.Pool, config: dict) -> None:
    ...
    feed_templates: list[str] = (
        list(feed_templates_raw)
        if not isinstance(feed_templates_raw, str)
        else [feed_templates_raw]
    )
    ...
    async with httpx.AsyncClient(
        headers={"User-Agent": "Mozilla/5.0 (compatible; BobMonitor/1.0)"}
    ) as client:
        if source == "static":
            logger.info(
                "Config %d (static): %d feeds, %d keywords, %d seen",
                config_id,
                len(feed_templates),
                len(keywords),
                len(seen),
            ...
            logger.info(
                "Config %d (agent): %d items, %d keywords, %d seen",
                config_id,
                len(watchlist),
                len(keywords),
                len(seen),
            ...
            for item_key, (item_name, item_date) in watchlist.items():
                ...
                for template in feed_templates:
                    feed_url = template.format(
                        query=query, key=item_key, name=item_name
                    )
                    await _process_feed_items(
                        pool,
                        client,
