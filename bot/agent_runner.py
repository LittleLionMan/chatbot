...
async def _handle_router_match(step: dict, context: dict[str, str], **_) -> str:
...
async def _handle_llm_step(
    ...
    **_,
    ...
    capability = (
        CAPABILITY_SIMPLE_TASKS if is_output else _LLM_CAPABILITY_MAP[step_type]
    )
    prompt = _resolve_template(step.get("prompt", ""), context)
    result = await brain.chat(
...
async def _handle_state_read(
    step: dict, context: dict[str, str], state: dict[str, str], **_
) -> str:
    key: str = step["key"]
...
async def _handle_data_read(
    step: dict, context: dict[str, str], pool: asyncpg.Pool, agent_id: int, **_
) -> str:
...
async def _handle_data_read_external(
    step: dict, context: dict[str, str], pool: asyncpg.Pool, **_
) -> str:
...
async def _handle_data_write_external(
    ...
    logger.info(
        "data_write_external: %s → %s/%s (%d chars)",
        agent_name,
        namespace,
        key,
        len(value),
...
def _transform_array_push(step: dict, context: dict[str, str]) -> str:
    ...
    if not value or not group:
        logger.warning(
            "transform array_push: value_key %r or group_key %r is empty",
            value_key,
            group_key,
        )
    ...
    logger.info(
        "transform array_push: %s[%r] now %d entries",
        target_key,
        group,
        len(result[group]),
    )
...
def _transform_map_field(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform map_field: source_key %r is not a valid JSON array", source_key
        )
        return "[]"
    values = [
...
def _transform_filter(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform filter: source_key %r is not a valid JSON array", source_key
        )
        return "[]"
    result = [
        item for item in data if all(_matches_item_filter(item, f) for f in filters)
    ]
...
def _transform_union(step: dict, context: dict[str, str]) -> str:
    ...
    for item in source_items + other_items:
        ...
        if key not in seen:
            ...
            result.append(item)
    logger.info(
        "transform union: %d + %d = %d unique items",
        len(source_items),
        len(other_items),
        len(result),
    )
...
def _transform_group_by(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform group_by: source_key %r is not a valid JSON array", source_key
        )
        return "{}"
...
def _transform_flatten(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform flatten: source_key %r is not a valid JSON array", source_key
        )
        return "[]"
    result: list = []
    for item in data:
...
def _transform_sort(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform sort: source_key %r is not a valid JSON array", source_key
        )
        return "[]"
    try:
        if field:
            data.sort(
                key=lambda x: x.get(field, "") if isinstance(x, dict) else "",
                reverse=reverse,
            )
        else:
...
def _transform_statistics(step: dict, context: dict[str, str]) -> str:
    ...
    functions: list[str] = step.get(
        "functions",
        ["count", "mean", "median", "q1", "q3", "iqr", "lower_bound", "upper_bound"],
    )
    ...
    try:
        ...
        logger.warning(
            "transform statistics: source %r is not a valid dict", source_key
        )
        return "{}"
    ...
    def _calc(prices: list) -> dict:
        ...
        if "std_dev" in functions:
            result["std_dev"] = (
                round(statistics.stdev(floats), 2) if count >= 2 else None
            )
        if "min" in functions:
        ...
        if any(
            f in functions for f in ["q1", "q3", "iqr", "lower_bound", "upper_bound"]
        ):
            if count >= 4:
...
def _transform_json_path(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform json_path: could not extract %r from %r", path, source_key
        )
        return step.get("default", "")
...
def _transform_xml_extract(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        elements = (
            root.findall(xpath, namespaces) if namespaces else root.findall(xpath)
        )
        if not elements:
        ...
        el = elements[0]
        result = (
            el.get(attribute, step.get("default", ""))
            if attribute
            else (el.text or step.get("default", ""))
        )
...
def _transform_regex_extract(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        if not match:
            logger.warning(
                "transform regex_extract: pattern %r found no match", pattern
            )
            return step.get("default", "")
...
def _transform_arithmetic(step: dict, context: dict[str, str]) -> str:
    ...
    for token in sorted(tokens, key=len, reverse=True):
        ...
        if val == "":
            logger.warning(
                "transform arithmetic: variable %r not found in context", token
            )
            return default
        try:
            float(val)
            ...
            logger.warning(
                "transform arithmetic: variable %r is not numeric: %r", token, val
            )
            return default
    ...
    if not all(c in allowed for c in resolved):
        logger.warning(
            "transform arithmetic: expression contains invalid characters: %r", resolved
        )
        return default
    try:
...
def _transform_compare(step: dict, context: dict[str, str]) -> str:
    ...
    try:
        ...
        logger.warning(
            "transform compare: could not parse %r=%r or %r=%r as float",
            left_key,
            left_raw,
            right_key,
            right_raw,
...
async def _handle_transform(step: dict, context: dict[str, str], **_) -> str:
...
async def _handle_trigger_agent(
    ...
    payload = {
        k: _resolve_template(str(v), context) for k, v in payload_template.items()
    }
    if target_name:
    ...
    **_,
    ...
    if condition_key:
        ...
        try:
            ...
            if not parsed:
                logger.info(
                    "notify_user: condition_key %r is empty/falsy, skipping",
                    condition_key,
                )
                return ""
        except Exception:
            if not condition_val:
                logger.info(
                    "notify_user: condition_key %r is empty, skipping", condition_key
                )
                return ""
    ...
    message_text = (
        _get(context, source_key)
        if source_key
        else _resolve_template(message_template, context)
    )

    if message_text:
        sent = await bot.send_message(chat_id=target_chat_id, text=message_text)
        await memory.save_message_annotated(
            pool,
        ...
        try:
            await memory.save_agent_notification(
                ...
                payload_summary={"summary": message_text[:200]},
            )
        except Exception as notif_err:
            logger.warning("notify_user: failed to save notification: %s", notif_err)
        logger.info("notify_user: sent %d chars", len(message_text))
    return ""
            ...
            if tool == "notify_user":
                ...
                if msg:
                    ...
                    try:
                        ...
                        logger.warning(
                            "tool notify_user: failed to save notification: %s",
                            notif_err,
                        )
                ...
                if target_name:
                    await memory.enqueue_agent_trigger(
                        pool, agent_id, target_name, payload, delay
                    )
...
async def _execute_pipeline(
    ...
    for step in steps:
        ...
        if not _route_allows(step, active_route):
            logger.info(
                "agent %s step %r skipped (route=%s)", name, step_id, active_route
            )
            continue
        ...
        if not _key_condition_allows(step, context):
            only_if_key = step.get("only_if_key", {})
            logger.info(
                "agent %s step %r skipped (key condition: %s != %s)",
                name,
                step_id,
                only_if_key.get("key"),
                only_if_key.get("value"),
            )
            continue
        ...
        if handler is None:
            logger.warning(
                "agent %s step %r: unknown type %r", name, step_id, step_type
            )
            continue
    ...
    for read in data_reads:
        ...
        if read_type == "state":
            ...
            if state is None:
                continue
            combined = "\n".join(
                f"{k}: {v}" for k, v in state.items() if k != "last_run_summary"
            )
            if combined:
            ...
            if not key:
                rows = await memory.query_agent_data(
                    pool, namespace, agent_id=target_agent_id
                )
                if rows:
                ...
                value = await memory.read_agent_data(
                    pool, target_agent_id, namespace, resolved_key
                )
                if value is not None:
                    label = (
                        read.get("as")
                        or f"db:{agent_name or 'self'}:{namespace}:{resolved_key}"
                    )
                    result[label] = value
    return result
...
async def execute_agent(
    ...
    try:
        ...
        flat_payload: dict[str, str] = {
            k: str(v) for k, v in (trigger_payload or {}).items()
        }
        ...
        steps: list[dict] = config_data.get("steps") or config_data.get(
            "pipeline", []
        ) + config_data.get("pipeline_after_template", [])
        ...
        if has_output_step:
            try:
                ...
                logger.warning(
                    "agent %s output step JSON parse failed: %r",
                    name,
                    output_step_result[:300],
                )
                parsed_output = {
                    "report": output_step_result,
                    "notify_user": True,
                    "tool_calls": [],
                }
        else:
            raw_structured = await brain.chat(
                ...
                messages=[
                    {"role": "user", "content": output_step_result or "Keine Ausgabe."}
                ],
                capability=CAPABILITY_SIMPLE_TASKS,
        ...
        if (
            ...
            and report
            and report.strip() != "KEINE_AENDERUNG"
        ):
            ...
            try:
                ...
                logger.warning(
                    "agent %s: failed to save notification: %s", name, notif_err
                )
